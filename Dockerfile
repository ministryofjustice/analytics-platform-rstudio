FROM continuumio/miniconda3:4.8.2 as miniconda3
...
ENV R_VERSION=3.5.1 \
    ...
    SIMBAATHENA_MAJOR_VERSION=1.0.3 \
    SIMBAATHENA_PATCH_VERSION=1004-1
...
RUN curl -s "https://s3.amazonaws.com/athena-downloads/drivers/ODBC/SimbaAthenaODBC_${SIMBAATHENA_MAJOR_VERSION}/Linux/simbaathena-${SIMBAATHENA_MAJOR_VERSION}.${SIMBAATHENA_PATCH_VERSION}.x86_64.rpm" > "/tmp/simbaathena.rpm" \
    && alien -i "/tmp/simbaathena.rpm" \
    && rm -f "/tmp/simbaathena.rpm"
COPY files/odbc* /etc/
